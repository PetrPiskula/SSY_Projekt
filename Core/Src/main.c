...
/* USER CODE BEGIN PFP */
extern void APP_BSP_Button1Action(void);
/* USER CODE END PFP */
...
int main(void)
{
  ...
  while (1)
  {
      uint8_t obstacle = (HAL_GPIO_ReadPin(GPIOB, GPIO_PIN_2) == GPIO_PIN_RESET);

      if (obstacle != last_state)
      {
          last_state = obstacle;
          ...
          {
              BSP_LED_On(LED_RED);
              printf("STAV: MAGNET! -> volam Button1Action a blikam LEDkou\r\n");
          }
          else
          {
              BSP_LED_Off(LED_RED);
              printf("STAV: zadny_magnet.. -> volam Button1Action a blikam LEDkou\r\n");
          }

          APP_BSP_Button1Action();  // odešle Toggle na koordinátora
      }
